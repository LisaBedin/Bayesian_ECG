...
from posterior_samplers.red_diff.algos.reddiff import REDDIFF
from posterior_samplers.red_diff.models.classifier_guidance_model import ClassifierGuidanceModel, Diffusion
...
def main(cfg: DictConfig) -> None:
    ...
    for x_orig, labels in tqdm(testloader, total=len(testloader)):
        ...
        clfg = ClassifierGuidanceModel(
            ...
            classifier=None,
            diffusion=Diffusion(device=torch.device('cuda')),
            cfg=None,
        )
        reddiff_cfg = DictConfig({"algo": cfg.algo.reddiff})
        rdiff = REDDIFF(clfg, cfg=reddiff_cfg, H=H_func)
        samples = rdiff.sample(
            initial_noise, None, epsilon_net.timesteps, labels=labels, obs=H_func.H(x_orig[0:1]).reshape(1, -1)
        ).detach()
        ...
        np.savez(os.path.join(results_path, f'reddiff_seed{cfg.algo.seed}_1000.npz'),
