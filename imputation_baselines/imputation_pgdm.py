...
def main(cfg: DictConfig) -> None:
    ...
    for x_orig, labels in tqdm(testloader, total=len(testloader)):
        ...
        samples = pgdm_svd(
            initial_noise, labels,  # .repeat((cfg.algo.nsamples, 1)),
            epsilon_net_svd, obs, H_func, obs_std
        )
        ...
        np.savez(os.path.join(results_path, f'PGDM_seed{cfg.algo.seed}_GOOD.npz'),
