...
def train(model, config, train_loader, device, valid_loader=None, valid_epoch_interval=5, foldername=""):
    ...
    for epoch_no in range(config["epochs"]):
        ...
        with tqdm(train_loader) as it:
            for batch_no, (clean_batch, noisy_batch, _) in enumerate(it, start=1):
                clean_batch, noisy_batch = clean_batch.to(torch.float32).to(device), noisy_batch.to(torch.float32).to(device)
                optimizer.zero_grad()
