...
from preprocessing.physionet_tools import find_challenge_files, get_filtered_ecg_from_header, get_labels, get_recording_id, get_age, get_sex
import numpy as np
from preprocessing.db_orm import Record
...
def get_beat_ecg_from_headers(x, Dx_map):
    ...
    ecg = get_filtered_ecg_from_header(header, leads_in_order, recording_file, final_freq=100)
...
if __name__ == '__main__':
    ...
    data_source = '/mnt/data/gabriel/physionet.org/files/challenge-2021/1.0.3/training/georgia'
    data_destination = '/mnt/data/lisa/physionet.org/georgia_100Hz'  # '/mnt/data/lisa/physionet.org/ecg_10s_db'  # sys.argv[2]
    ...
    with Session(engine) as session:
        for database in available_databases:
            ...
            results_path = os.path.join(data_destination, database)
            generate_data_fun = partial(generate_data_for_db, path=results_path)

            if n_procs > 0:
                with Pool(n_procs) as p:
                    batch_meta = p.map(func=generate_data_fun, iterable=find_challenge_files(database_path))
            else:
                list_f = find_challenge_files(database_path)
