## Modified based on https://github.com/helme/ecg_ptbxl_benchmarking
...
def resample_poly_fn(recording, final_freq, freq):
    L = math.ceil(len(recording.T) * final_freq / freq)
    normECG = list(reversed(recording.T)) + list(recording.T) + list(reversed(recording.T))
    # resample beat by beat and saving it
    res = resample_poly(normECG, final_freq, freq)
    res = res[L - 1:2 * L - 1]
    return res.T
...
def filter_ecg(final_freq, freq, original_recording):
    ...
    T = original_recording.shape[1]
    padded_original_recording = np.concatenate((original_recording[:, ::-1], original_recording, original_recording[:, ::-1]), axis=1)
    ...
    filtered_recording = sosfiltfilt(high_pass_filter,
                                     ...
                                     axis=-1)[:, T:2*T]
    if freq != final_freq:
        # recording = nnresample_fn(filtered_recording,  # Very long !!
        #                        final_freq,
        #                        int(freq),
        #                         axis=1)
        recording = resample_poly_fn(filtered_recording, final_freq, freq)
    else:
        recording = filtered_recording.copy()
    return recording


def load_codes(path):  # , sampling_rate, release=False):
...
def select_data(YY, ctype, min_samples, outputfolder):
