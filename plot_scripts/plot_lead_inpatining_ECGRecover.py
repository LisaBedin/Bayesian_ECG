...
from EcgRecover.tools.LoadModel import load_model
...
def main():
    ...
    ae_path = '/mnt/data/lisa/ecg_results/ECGrecover'
    checkpoint = torch.load(os.path.join(ae_path, 'Model.pth'))

    Autoencoder.load_state_dict(checkpoint)
    Autoencoder = Autoencoder.to('cuda:0')
    Xc = torch.Tensor(np.concatenate([y, np.zeros((*y.shape[:2], 24))], axis=-1)).cuda().unsqueeze(1)
    with torch.no_grad():
        y_hat = Autoencoder(Xc, 'cuda:0').detach().cpu().numpy()[:, :, :1000]

    print(y_hat.shape)
    mask = get_12leads_mask(npz['mask'][inds, :, :T])
    mae_per_test = MAE(y, y_hat, mask)
    rmse_per_test = RMSE(y, y_hat, mask)
    ...
    cos_per_test = cosine_similarity(y, y_hat, mask)
    ...
    dtw_per_test = sdtw(torch.tensor(y[:, 1:]), torch.tensor(y_hat[:, 1:])).numpy() / 1024
    ...
    pred_scores = model.predict(np.swapaxes((y_hat-mean_train) / std_train, 1, 2))
