...
def main():
    ...
    for prefix in ['seed0_GOOD', 'DPS_seed0', 'PGDM_seed0', 'DDNM_seed0', 'diffpir_seed0', 'reddiff_seed0_1000', 'repaint_seed0']:
        print(f'# ================== {prefix} ============== # ')
        ...
        if dataset == 'ptbxl':
            lab_names = ['1AVB', 'RBBB', 'LBBB', 'SB', 'AF', 'ST']  # 1AVB, SBRAD, ST ? 0, 59, 47/
            pred_gen = pred_gen[:, :, np.array([0, 1, 2, 3, 4, 5]).astype(int)]
            pred_real = pred_real[:, np.array([0, 1, 2, 3, 4, 5]).astype(int)]
            real_labs = labels[:, np.array([0, 12, 11, 59, 4, 47]).astype(int)]  # RBBB, LBBB, AF
        else:
        ...
        # opt_th = 0.5 * np.ones_like(opt_th)
    
        database_path = '/mnt/Reseau/Signal/lisa/physionet.org/files/codetest'
        df_exams = pd.read_csv(os.path.join(database_path, 'exams.csv'))
        all_h5py = glob.glob(os.path.join(database_path, 'exams*.hdf5'))
        val_pred_lst, val_lab_lst = [], []
        #for file_p in all_h5py:
        all_opt_th = []
        file_p = all_h5py[0]
        with h5py.File(file_p, 'r') as f_:
            val_pred = model.predict(np.array(f_['tracings']))
            val_ids = np.array(f_['exam_id']).astype(int)
        #cval_pred = np.concatenate(val_pred, axis=0)  # [:, 48:4096 - 48]
        #val_ids = np.concatenate(val_ids, axis=0).astype(int)
        val_labs = [np.array(df_exams[df_exams['exam_id'] == id_][['1dAVb', 'RBBB', 'LBBB', 'SB', 'AF', 'ST']]).astype(int) for id_ in val_ids]
        kept_ids = np.array([k for k in range(len(val_labs)) if val_labs[k].shape[0]>0])
        val_lab_lst.append(np.concatenate(val_labs)[kept_ids])
        val_pred_lst.append(val_pred[kept_ids])
        opt_prec, opt_recall, opt_th = get_optimal_precision_recall(val_lab_lst[-1], val_pred_lst[-1])
        all_opt_th.append(opt_th)
    
        # opt_th = 0.5 * np.ones_like(opt_th)
        opt_th = np.array([0.336957, 0.53175, 0.479983, 0.409251, 0.402606, 0.459091], dtype=np.float32)
    
        print('# ================ F1 ================ #')
        gen_scores = [f1_score(real_labs[:, k], (np.mean(pred_gen[:, :, k], axis=1) > opt_th[k]).astype(int)) for k in range(real_labs.shape[1])]
        ...
        print('# ================ bacc ================ #')
        gen_scores = [balanced_accuracy_score(real_labs[:, k], (np.mean(pred_gen[:, :, k], axis=1) > opt_th[k]).astype(int)) for k in range(real_labs.shape[1])]
        real_scores = [balanced_accuracy_score(real_labs[:, k], (pred_real[:, k] > opt_th[k]).astype(int)) for k in range(real_labs.shape[1])]
        for k in range(real_labs.shape[1]):
            print(f'{lab_names[k]}: real={real_scores[k]:.3f}, pred={gen_scores[k]:.3f}')
    
        print('# ================ recall ================ #')
        gen_scores = [recall_score(real_labs[:, k], (np.mean(pred_gen[:, :, k], axis=1) > opt_th[k]).astype(int)) for k in range(real_labs.shape[1])]
        real_scores = [recall_score(real_labs[:, k], (pred_real[:, k] > opt_th[k]).astype(int)) for k in range(real_labs.shape[1])]
        for k in range(real_labs.shape[1]):
            print(f'{lab_names[k]}: real={real_scores[k]:.3f}, pred={gen_scores[k]:.3f}')
    
        print('# ================ specificity ================ #')
        gen_scores = [recall_score(real_labs[:, k], (np.mean(pred_gen[:, :, k], axis=1) > opt_th[k]).astype(int), pos_label=0) for k in range(real_labs.shape[1])]
        real_scores = [recall_score(real_labs[:, k], (pred_real[:, k] > opt_th[k]).astype(int), pos_label=0) for k in range(real_labs.shape[1])]
        for k in range(real_labs.shape[1]):
            print(f'{lab_names[k]}: real={real_scores[k]:.3f}, pred={gen_scores[k]:.3f}')
    
        print('# ================ precision ================ #')
        gen_scores = [precision_score(real_labs[:, k], (np.mean(pred_gen[:, :, k], axis=1) > opt_th[k]).astype(int)) for k in range(real_labs.shape[1])]
        real_scores = [precision_score(real_labs[:, k], (pred_real[:, k] > opt_th[k]).astype(int)) for k in range(real_labs.shape[1])]
        for k in range(real_labs.shape[1]):
            print(f'{lab_names[k]}: real={real_scores[k]:.3f}, pred={gen_scores[k]:.3f}')
    
    
        print('# ================ auc ================ #')
        gen_scores = [roc_auc_score(real_labs[:, k], np.mean(pred_gen[:, :, k], axis=1)) for k in
                      range(real_labs.shape[1])]
        real_scores = [roc_auc_score(real_labs[:, k], pred_real[:, k]) for k in range(real_labs.shape[1])]
        for k in range(real_labs.shape[1]):
            print(f'{lab_names[k]}: real={real_scores[k]:.3f}, pred={gen_scores[k]:.3f}')
