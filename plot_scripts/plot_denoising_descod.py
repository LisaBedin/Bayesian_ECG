...
def main():
    ...
    # =========== read DeScoD model =============== #
    path = "../DeScoD/config/base.yaml"
    with open(path, "r") as f:
        config = yaml.safe_load(f)
    base_model = ConditionalModel(config['train']['feats']).to('cuda:0')
    modelBW = DDPM(base_model, config, 'cuda:0').cuda()

    foldername = '/mnt/data/lisa/ecg_results/DeScoD/checkpoints/noise_type_bw0'
    checkpoint = torch.load(os.path.join(foldername, 'model.pth'))
    modelBW.load_state_dict(checkpoint)
    ...
    for lab in ['NSR', 'LBBB', 'RBBB', 'AF']:
        ...
        X_inp = torch.Tensor(npz_bw['noisy']).cuda()
        with torch.no_grad():
            ecg_bw = np.zeros(X_inp.shape)
            for k in tqdm(range(1)):
                with torch.no_grad():
                    ecg_bw += modelBW.denoising(X_inp).detach().cpu().numpy()
            ecg_bw = get_12leads(ecg_bw[:, :, :1000]) #/10)
        X_inp = torch.Tensor(npz_em['noisy']).cuda()
        with torch.no_grad():
            ecg_em = np.zeros(X_inp.shape)
            for k in tqdm(range(1)):
                with torch.no_grad():
                    ecg_em += modelBW.denoising(X_inp).detach().cpu().numpy()
            ecg_em = get_12leads(ecg_em[:, :, :1000]) # /10)
        ...
        all_ecg_em.append(ecg_em)
        all_ecg_bw.append(ecg_bw)
    ...
    mae_per_test = MAE(all_ecg_real, all_ecg_em)
    rmse_per_test = RMSE(all_ecg_real, all_ecg_em)
    ...
    dtw_per_test = sdtw(torch.tensor(all_ecg_real), torch.tensor(all_ecg_em)).numpy() / 1024
    ...
    mae_per_test = MAE(all_ecg_real, all_ecg_bw)
    rmse_per_test = RMSE(all_ecg_real, all_ecg_bw)
    CI_mae = stats.t.interval(0.95, len(mae_per_test) - 1, loc=mae_per_test.mean(), scale=stats.sem(mae_per_test))
    CI_rmse = stats.t.interval(0.95, len(rmse_per_test) - 1, loc=rmse_per_test.mean(), scale=stats.sem(rmse_per_test))
    print('MAE', mae_per_test.mean(), '+/-', abs(CI_mae[1] - CI_mae[0]) / 2.)
    print('RMSE', rmse_per_test.mean(), '+/-', abs(CI_rmse[1] - CI_rmse[0]) / 2.)
    sdtw = SoftDTW(use_cuda=False, gamma=0.1)
    dtw_per_test = sdtw(torch.tensor(all_ecg_real), torch.tensor(all_ecg_bw)).numpy() / 1024
    CI_dtw = stats.t.interval(0.95, len(dtw_per_test) - 1, loc=dtw_per_test.mean(), scale=stats.sem(dtw_per_test))
    print('DTW', dtw_per_test.mean(), '+/-', abs(CI_dtw[1] - CI_dtw[0]) / 2.)
