...
# oes.environ['CUDA_VISIBLE_DEVICES'] = '5'
...
def train(
    ...
    mel_path (str):                 for vocoding, path to mel spectrograms ( generate these on the fly)
    """
    ...
    while n_iter < n_iters + 1:
        ...
        for data in tqdm(trainloader, desc=f'Epoch {n_iter // len(trainloader)}'):
            if 'physionet' in dataset_cfg._name_:
                ...
                label = None
            elif 'ptbxl' in dataset_cfg._name_:
                audio, label = data
                audio, label = audio.cuda(), label.to(torch.float32).cuda()
            elif 'large' in dataset_cfg._name_:
                audio, label = data
                audio, label = audio.cuda(), label.to(torch.float32).cuda()
                # generate pseudo labels, since the classifier is trained on ptb-xl training set, the labels should be identical as the input
                ...
                label = mel_spectrogram.cuda()
                audio = audio.cuda()

            # back-propagation
            optimizer.zero_grad()
            loss = training_loss(net, nn.MSELoss(), audio, diffusion_hyperparams, label=label)
            if num_gpus > 1:
            ...
            if n_iter % iters_per_ckpt == 0 and rank == 0:
                ...
                with open(os.path.join(checkpoint_directory, 'epoch.txt'), 'w') as file:
                    file.write(str(n_iter))
            ...
            if n_iter % iters_per_logging == 0 and rank == 0:
                ...
                samples = generate(
                    ...
                    name=name,
                    label=label[:generate_cfg['n_samples']] if not model_cfg["unconditional"] else None,
                    **generate_cfg,
...
def training_loss(net, loss_fn, audio, diffusion_hyperparams, label=None):
    """
    ...
    epsilon_theta = net((transformed_X, diffusion_steps.view(B,1),), label=label)  # predict \epsilon according to \epsilon_\theta
...
def main(cfg: DictConfig) -> None:
    ...
    os.makedirs(cfg.train.results_path, mode=0o775, exist_ok=True)  # folder to save the experiments
