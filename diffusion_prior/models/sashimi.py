...
from diffusion_prior.models.utils import calc_diffusion_step_embedding
from diffusion_prior.models.s4 import S4
...
class DiffWaveBlock(nn.Module):
    def __init__(self,
        ...
        if not self.unconditional:
            self.fc_label = nn.Linear(label_embed_dim, self.d_model)
    ...
    def forward(self, x, diffusion_step_embed, label_embed=None):
        y = x
        B, C, L = x.shape
        ...
        # part_t = part_t.view([B, self.d_model, 1]) # should be unsqueeze?
...
class Sashimi(nn.Module):
    def __init__(self, in_channels=1, out_channels=1,
        ...
        # unconditional = label_embed_classes == 0
        self.unconditional = unconditional
        # embedding for global conditioning
        self.embedding = nn.Embedding(label_embed_classes,
                                      label_embed_dim) if not unconditional else None
        ...
        for p in pool[::-1]:
            H //= expand
            L *= p
            u_layers.append(UpPool(H*expand, H, pool=p))
    ...
    def forward(self, input_data, label=None):
        ...
        for layer in self.d_layers:
            outputs.append(x)
            x = layer(x, diffusion_step_embed, label_embed=label_embed)
    ...
    def __repr__(self):
        return f"sashimi_h{self.d_model}_d{self.n_layers}_pool{''.join([str(p_) for p_ in self.pool])}_expand{self.expand}_ff{self.ff}_{'uncond' if self.unconditional else 'cond'}"
