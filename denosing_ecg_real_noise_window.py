...
def main(cfg: DictConfig) -> None:
    ...
    for k in range(9):
        ...
        all_RR.append(RR)
    RR = np.median(all_RR)
    cfg.denoising.f_c_max = 30  # max(20, int(round(100-10/11*round(RR))))
    print('f_c_max', cfg.denoising.f_c_max)# 20  #40  # 10
    J = 76 # < 256 x 20 # 0 # int(cfg.denoising.f_c_max / 10 * 10000)  # 20000 # : which parameter ? 1000 cfg.denoising.J # number of harmonics
    phi = np.concatenate([np.cos(np.arange(cfg.dataset.segment_length)[:, None] /
                                 f_s * (np.arange(J)[None] / J *
                              (cfg.denoising.f_c_max - cfg.denoising.f_c_min)
                              +cfg.denoising.f_c_min)),
                           np.sin(np.arange(cfg.dataset.segment_length)[:, None] / f_s * np.arange(J)[None] / J * (
                                   (cfg.denoising.f_c_max - cfg.denoising.f_c_min)
                                   + cfg.denoising.f_c_min)
                                   )],
                          axis=1).T
    ...
    overlap = 128
    all_samples, all_phi = [], []
    for l_start in range(0, 1024-256, overlap):
        ...
        noisy_obs_patch = noisy_obs[:, :, :phi.shape[-1]]
        x_orig_patch = x_orig[:, :, :phi.shape[-1]]
        obs_std = torch.tensor(np.std(noisy_obs_patch[0].cpu().numpy(), axis=1)[None, :,
                               None]).cuda()  # .5 # we start with a large variance to allow to generate a signal far away from the observation
        solver = prox_lasso
        print(obs_std)
        reg = 1  # 1.  # we start with small regularization to allow to remove a lot of noise even if we end-up with a flat signal
        k = 0
        for k in range(6):
            obs = (noisy_obs_patch-cos_sin_proj(phi, eta_param)).cuda()
            # obs_std = new_obs_std
            def log_pot(x):
                b = obs.shape[0]
            ...
            samples = vdps_fn(
                ...
                ground_truth=x_orig_patch.cpu()
            )  # 10 minutes for 1 sample 1 observation...
            display_time_series(samples[0], gt=noisy_obs_patch[0])
            display_time_series(samples[0], gt=x_orig_patch[0])

            new_eta_param = []
            phi_estim = jnp.array(phi)
            for l in range(samples.shape[1]):
                y = jnp.array(noisy_obs_patch[:, l] - samples[:, l].detach().cpu())
                # y = noisy_obs_patch[:, l].to(samples.device) - samples[:, l]
                pg = ProximalGradient(fun=least_squares_jax, prox=solver, tol=0.0001, maxiter=1000, maxls=100, decrease_factor=0.9)
                # reg = 10.
            ...
            print(eta_param[0])
            display_time_series(noisy_obs_patch[0] - cos_sin_proj(phi, eta_param), gt=x_orig_patch[0])
            #if k > 2:
            #   reg = 5.
            if k >= 0:
                obs_std = np.round(
                    np.nanstd(noisy_obs_patch - samples.detach().cpu() - cos_sin_proj(phi, eta_param[0])[None],
                               axis=(0, 2)),
                    2)
                #if k ==1:
                #    obs_std /= 4.  # EM
                ...
                print(obs_std)
                obs_std = torch.tensor(obs_std[None, :, None]).cuda()
                #reg = 0.1
            if k == 2:
