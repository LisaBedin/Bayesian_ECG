...
from posterior_samplers.red_diff.models.classifier_guidance_model import ClassifierGuidanceModel
from posterior_samplers.red_diff.utils.degredations import build_degredation_model
...
class REDDIFF(DDIM):
    ...
    def sample(self, x, y, ts, labels=None, **kwargs):
        y_0 = kwargs["obs"]
        ...
        for ti, si in zip(reversed(ts), reversed(ss)):
            ...
            alpha_t = self.diffusion.alpha(t).view(-1, 1, 1)
            alpha_s = self.diffusion.alpha(s).view(-1, 1, 1)
            ...
            et, x0_hat = self.model(xt, y, t, labels, scale=scale)  # et, x0_pred
    ...
    def initialize(self, x, y, ts, **kwargs):
        ...
        alpha_t = self.diffusion.alpha(t).view(-1, 1, 1)  # it is zero
