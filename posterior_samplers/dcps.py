...
def dcps_step(
    ...
    if optimizer == "SGD":
        for _ in range(gradient_steps):
            ...
            mean = normalized_grad_step(mean, mean_grad, lr=lr)
            log_std = normalized_grad_step(log_std, logstd_grad, lr=lr)

    if optimizer == "Adam":
        mean, log_std = mean.requires_grad_(), log_std.requires_grad_()
        optim = torch.optim.Adam(lr=lr, params=[mean, log_std])
        for _ in range(gradient_steps):
            optim.zero_grad()
            kld = kl(mean, log_std)
            kld.backward()
    ...
    for idx, t_obs in enumerate(range(len(obs_timesteps) - 1, 0, -1)):
        ...
        for i in tqdm.tqdm(
            ...
            if display_im and i % display_freq == 0:
                ...
                display_fn(img, gt=ground_truth)

            if returns_all_x_t:
                sequence_x_t.append(x_tprev.cpu())
                sequence_t.append(t_prev)

    if returns_all_x_t:
        return x_tprev, sequence_x_t, sequence_t
