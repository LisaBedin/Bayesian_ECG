...
def display(x, save_path=None, title=None, gt=None):
    sample = x.squeeze(0).cpu().permute(1, 2, 0).clamp(-1, 1)
...
def display_time_series(x, save_path=None, gt=None):
    sample = x.squeeze(0).cpu()  # n_channels x signal_length
    # sample = sample / torch.max(torch.absolute(sample), axis=-1).unsqueeze(-1)
    fig, ax = plt.subplots(1, 1, figsize=(5, 8))
    fig.subplots_adjust(left=0.01, right=.99, top=.99, bottom=.01)
    if gt is not None:
        # gt = gt / torch.max(torch.absolute(gt), axis=-1).unsqueeze(-1)
        for i, track in enumerate(gt):
            ax.plot(track - i, color='red', alpha=.7)
