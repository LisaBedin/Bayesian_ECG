...
class InpaintingTimeSeries(H_functions):
    def __init__(self, n_leads, signal_length, missing_indices, device):
        # self.channels = channels
        self.L = n_leads
        self.channels = n_leads
        self.T = signal_length
        self._singulars = torch.ones(n_leads * signal_length - missing_indices.shape[0]).to(device)
        self.missing_indices = missing_indices
        self.kept_indices = torch.Tensor([i for i in range(n_leads * signal_length) if i not in missing_indices]).to(
            device).long()
    ...
    def Vt(self, vec):
        temp = vec.clone().reshape(vec.shape[0], -1)  # , self.channels, -1).permute(0, 2, 1).reshape(vec.shape[0], -1)
        out = torch.zeros_like(temp)
        out[:, :self.kept_indices.shape[0]] = temp[:, self.kept_indices]
        out[:, self.kept_indices.shape[0]:] = temp[:, self.missing_indices]
        return out
    ...
    def add_zeros(self, vec):
        temp = torch.zeros((vec.shape[0], self.L * self.T), device=vec.device)
        reshaped = vec.clone().reshape(vec.shape[0], -1)
        temp[:, :reshaped.shape[1]] = reshaped
        return temp
