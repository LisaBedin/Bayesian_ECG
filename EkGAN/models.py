...
if __name__ == "__main__":
    batch_size = 8
    time_len = 512
    input_shape = (batch_size, 1, 16, time_len)

    ig = InferenceGenerator()
    lg = LabelGenerator()
    d = Discriminator()

    ig_input = torch.randn(input_shape)
    lg_input = torch.randn(input_shape)
    dis_input = torch.randn(input_shape)
    dis_target = torch.randn(input_shape)

    ig_output, ig_lv = ig(ig_input)
    lg_output, lg_lv = lg(lg_input)
    dis_output = d(torch.cat([dis_input, dis_target], axis=1))

    print(f"Inference Generator Output Shape: {ig_output.shape}")
    print(f"Label Generator Output Shape: {lg_output.shape}")
    print(f"Discriminator Output Shape: {dis_output.shape}")
