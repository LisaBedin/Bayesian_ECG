...
def filtering(Z):
    ...
    #resampled_signal = signal.resample(filtered_signal, int(len(filtered_signal) * (new_sampling_frequency / original_sampling_frequency)))
...
def pre_processing(path, seed):
    all_files_path = glob.glob(os.path.join(path,'*.npz'))
    data_true = np.stack([np.concatenate([np.load(path_f)['ecg'], np.zeros((12, 24))], axis=1) for path_f in all_files_path])

    min_data = (data_true.max(axis=-1) - data_true.min(axis=-1)).min(axis=-1)
    data_true = data_true[min_data>1e-2]
    data_mask = np.zeros_like(data_true)
    data_mask[:, :1] = data_true[:, :1]
    #new_data = np.zeros((len(data),1024,12))
    #for i in tqdm(range(len(data))):
    #    for l in range(12):
    #        new_data[i,:,l] = filtering(data[i,:,l])
    # read_data has to be of shape n_data x 1024 x 12
