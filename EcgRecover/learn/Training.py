...
def load_Data(path_train, path_val, s, Verbose = False):
    X_train, X_true = pre_processing(path_train,seed = s)
    X_validation, X_validation_true = pre_processing(path_val,seed = s)
...
def training(train_set, train_label, validation_set, validation_label, epoch, batch_size, Autoencoder, device, optimizer, scheduler, loss, save_path=''):
    ...
    for e in tqdm(range(epoch)):
        ...
        for b in range(0,len(train_set), batch_size):
            ...
            #print(Xc.min(), Xc.max(), Xc.mean())
            lossMSE, corr, mse  = loss(label,Xc)
            if torch.isnan(lossMSE):
                print('NAN')
                import pdb
                pdb.set_trace()
            lossMSE.backward()
        ...
        torch.save(Autoencoder.state_dict(), os.path.join(save_path,"Modeltemp.pth"))
        it2 = 0 
    ...
    if save_path != '':
        torch.save(Autoencoder.state_dict(), os.path.join(save_path, "Modeltemp.pth"))
        np.save(os.path.join(save_path, "lost_list_train.npy"), lost_list_train)
        np.save(os.path.join(save_path, "lost_list_validation.npy"),lost_list_validation)
        np.save(os.path.join(save_path, "corr_list_train.npy"),corr_list_train)
        np.save(os.path.join(save_path, "corr_list_validation.npy"),corr_list_validation)
        np.save(os.path.join(save_path, "mse_list_train.npy"),mse_list_train)
        np.save(os.path.join(save_path, "mse_list_validation.npy"),mse_list_validation)
    
    #return(lost_list_train, lost_list_validation, corr_list_train, corr_list_validation,mse_list_train,mse_list_validation)


def Train_model(train_path, val_path, sd, dvc, epoch, batch_size, save_path = ''):

    ### Load Data ###
    print("Load Data")
    X_train, X_true,X_validation, X_validation_true = load_Data(train_path, val_path,sd)
